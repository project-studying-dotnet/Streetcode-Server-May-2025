...
public static class SeedingLocalExtension
{
    public static async Task SeedDataAsync(this WebApplication app)
    {
        using (var scope = app.Services.CreateScope())
        {
            ...
            await FillStorageBase(app, dbContext);
            await AddResponses(dbContext);
            await AddTerms(dbContext);
            await AddTeamMembersAndRelatedData(dbContext);
            await AddUsers(dbContext);
            await AddNews(dbContext);
            await AddStreetcodesAndRelatedData(dbContext);
        }
    }

    private static async Task FillStorageBase(WebApplication app, StreetcodeDbContext dbContext)
    {
        ...
        if (!dbContext.Images.Any())
        {
            ...
            foreach (var img in imgfromJson)
            {
                ...
                {
                    await blobService.SaveFileInStorageBase64Async(img.Base64, img.BlobName.Split('.')[0], img.BlobName.Split('.')[1]);
                }
            }
        ...
        if (!dbContext.Audios.Any())
        {
            ...
            foreach (var audio in audiosfromJson)
            {
                ...
                if (!File.Exists(filePath))
                {
                    await blobService.SaveFileInStorageBase64Async(audio.Base64, audio.BlobName.Split('.')[0], audio.BlobName.Split('.')[1]);
                }
            }
    ...
    private static async Task AddResponses(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTerms(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTeamMembersAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.TeamMembers.Any())
        {
            ...
            await AddPositionsAndRelatedData(dbContext);
        }
    }

    private static async Task AddPositionsAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.Positions.Any())
        {
            ...
            await AddTeamMemberPositions(dbContext);
            await AddTeamMemberLinks(dbContext);
        }
    }

    private static async Task AddTeamMemberPositions(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTeamMemberLinks(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddUsers(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddNews(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddStreetcodesAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.Streetcodes.Any())
        {
            ...
            await AddSubtitles(dbContext);
            await AddStreetcodeCoordinates(dbContext);
            await AddVideos(dbContext);
            await AddPartnersAndRelatedData(dbContext);
            await AddArtsAndRelatedData(dbContext);
            await AddTexts(dbContext);
            await AddTransactionLinks(dbContext);
            await AddTimelineItemsAndRelatedData(dbContext);
            await AddFacts(dbContext);
            await AddImageDetailses(dbContext);
            await AddRelatedFigures(dbContext);
            await AddStreetcodeImages(dbContext);
            await AddComments(dbContext);
            await AddSourceLinksAndRelatedData(dbContext);
            await AddTagsAndRelatedData(dbContext);
        }
    }

    private static async Task AddSubtitles(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddStreetcodeCoordinates(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddVideos(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddPartnersAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.Partners.Any())
        {
            ...
            await AddPartnerSourceLinks(dbContext);
            await AddStreetcodePartners(dbContext);
        }
    }

    private static async Task AddPartnerSourceLinks(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddStreetcodePartners(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddArtsAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.Arts.Any())
        {
            ...
            await AddStreetcodeArts(dbContext);
        }
    }

    private static async Task AddStreetcodeArts(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTexts(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTransactionLinks(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddFacts(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddImageDetailses(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTimelineItemsAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.TimelineItems.Any())
        {
            ...
            await AddHistoricalContextsAndRelatedData(dbContext);
        }
    }

    private static async Task AddHistoricalContextsAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.HistoricalContexts.Any())
        {
            ...
            await AddHistoricalContextTimelines(dbContext);
        }
    }

    private static async Task AddHistoricalContextTimelines(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddRelatedFigures(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddStreetcodeImages(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddComments(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddSourceLinksAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.SourceLinks.Any())
        {
            ...
            await AddStreetcodeCategoryContent(dbContext);
        }
    }

    private static async Task AddStreetcodeCategoryContent(StreetcodeDbContext dbContext)
    {
    ...
    private static async Task AddTagsAndRelatedData(StreetcodeDbContext dbContext)
    {
        if (!dbContext.Tags.Any())
        {
            ...
            await AddStreetcodeTagIndices(dbContext);
        }
    }

    private static async Task AddStreetcodeTagIndices(StreetcodeDbContext dbContext)
    {
